...
class APIClient{
  ...
  login(String username, String password, String mobile, String lat, String long)async{
    ...
    final response = await http.get(Uri.parse(baseUrl+'mRegisterPg.jsp?Str_iMeiNo=1&Str_Model=Android'
  ...
  setPin(String pin, String clientId)async{
    final header = buildHeader();
    final response = await http.get(Uri.parse(baseUrl+'mSetPinPg.jsp?Str_iMeiNo=1&Str_Model=IOS&'
  ...
  getPin(String pin)async{
    final header = buildHeader();
    final response = await http.get(Uri.parse(baseUrl+'mLoginPg.jsp?Str_iMeiNo=1&'
